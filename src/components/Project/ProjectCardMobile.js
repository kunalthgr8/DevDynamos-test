...
const ProjectCardMobile = (props) => {
    ...
    return (
        <motion.div className="project-container-mobile"
            ...
            onMouseLeave={() => setIsHovering(!isHovering)} animate={{background : isHovering ? "#16446F" : "", boxShadow: isHovering ? "-2px -2px 2px 0px white" : "0"}}>
            <motion.h1 className='project-title-mobile' animate={{background: isHovering ? "#16446F" : "gray"}}>{props.title}</motion.h1>
            <div className="transform-container-mobile">
                ...
                <motion.div className="content-mobile" animate={{ position: isHovering ? "relative" : "absolute", opacity : isHovering ? 1 : 0, color: isHovering ? "rgba(255, 255, 255)" : "rgba(255, 255, 255, 0)"}}>
                    ...
                    <motion.button className='project-btn-mobile'animate={{ width: isHovering ? "80%" : "50%", boxShadow: isHovering ? "0px 3px 5px 0px black" : "" }} transition={{delay: 0.5}}>Try Now</motion.button>
                </motion.div>
            </div>
        </motion.div>
    );
